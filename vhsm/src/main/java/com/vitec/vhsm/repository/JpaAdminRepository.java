...
public class JpaAdminRepository implements AdminRepository {
    ...
    public Optional<Admin> findByUserId(String user_id) {
        List<Admin> result = em.createQuery("select a from Admin a where a.user_id = :user_id", Admin.class)
    ...
    public Optional<Admin> findByName(String name) {
        List<Admin> result = em.createQuery("select a from Admin a where a.name = :name", Admin.class)
